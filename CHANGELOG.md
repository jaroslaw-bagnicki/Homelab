...
- **(docs)** Add ADR 27 — two-tier monitoring strategy reconciling Azure Monitor via Arc (Tier A, management plane) with Netdata on every node (Tier B); Grafana/Prometheus/Fluent Bit/Loki listed as Tier B extensions (no ADR yet); ADR 09 amended in place, ADR 24/25/26 aligned — [ADR 27](docs/decisions/27-monitoring-strategy.md) · [#75](https://github.com/jaroslaw-bagnicki/Homelab/issues/75)
