...
- **(docs)** Add ADR 30 + runbook 29 — UPS graceful shutdown via **NUT** on the HA node: the Green Cell UPSLM600 (`0665:5161`) stays on the Wyse 5070's Proxmox host, the NUT server runs in a dedicated unprivileged **LXC 103**, and the driver is **`nutdrv_qx`** (the unit's vendor-defined HID usage page rules out `usbhid-ups`); the hypervisor is the sole `upsmon` primary with `lab`/`edge` as secondaries, shutdown triggers on `LB`, and runbook 29 covers the two-strip power layout plus the modified-sine pull-the-plug test for the Beetle's active-PFC supply — [ADR 30](docs/decisions/30-ups-nut-graceful-shutdown.md) · [runbook 29](docs/runbooks/29-nut-ups-shutdown.md) · [#111](https://github.com/jaroslaw-bagnicki/Homelab/issues/111)
