...
- **(feat)** Dedicated fleet-wide SSH key for Ansible — `ansible-fleet@homelab` keypair generated by `scripts/New-HomelabFleetSshKey.ps1`, private key in `homelab-bysxdb-kv/ansible-fleet-key-priv`, public key committed at `ansible/roles/common/files/ssh/ansible-fleet.pub` and deployed to `labadmin` on every host by the `common` role (`ansible.posix.authorized_key`); `profile.ps1` loads it into `ssh-agent` alongside the VPS key — [ADR 28](docs/decisions/28-fleet-ssh-key.md)
