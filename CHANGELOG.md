...
- **(feat)** Dedicated fleet-wide SSH key for automation — `ansible-fleet@homelab` keypair generated by `scripts/New-HomelabFleetSshKey.ps1`, private key in `homelab-bysxdb-kv/ansible-fleet-key-priv`, public key committed at `ansible/roles/common/files/ssh/ansible-fleet.pub` — the breaking-glass bootstrap installs it into `labadmin` so it establishes the first connection (runbooks 24/25), and the `common` role re-arms it on every host (`ansible.posix.authorized_key`, restrictive `key_options`); `profile.ps1` loads it into `ssh-agent` alongside the VPS key — used by Ansible and AI agent tooling (OpenCode, Copilot) — [ADR 28](docs/decisions/28-fleet-ssh-key.md)
