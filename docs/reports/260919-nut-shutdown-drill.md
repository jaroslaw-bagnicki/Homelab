...
Sources named per row: **`ha` journal**, **`lab` journal**, **`edge` journal** and **LXC 213 journal** are each node's own journal for the drill boot (`journalctl -b -1`); **`ha` `upsc` poller** is a script on `ha` that read the UPS state every 5 s. `edge`'s journal is volatile, so its lines were copied to a file as they were produced — see Evidence. Every quotation below is verbatim from the named capture.
...
`edge`'s journal is the only record of that node's own view, and the timeline above uses the copy taken during the drill: `on battery` at 10:36:50, `battery is low` at 11:29:42, and the post-boot `multi-user.target` plus `nut-monitor` reconnect at 12:02:35. It does **not** cover the shutdown window — the copy jumps from 11:29:42 to 12:02:35. That is a limitation of the capture method (edge's journal is volatile), not a finding about the shutdown: the ordering is established from `ha`, `upsd` and `lab`, and does not depend on edge. A future drill on edge should set `Storage=persistent` for the window.
