...
|---|---|---|---|
| **SONOFF ZBDongle-P** | CC2652P (TI) | 95–99,90 PLN (~22–24 EUR) | **Pick — most mature/stable for Z2M (Z-Stack, gold standard)**; ext. antenna + aluminium RF-shielding case (95 PLN cheapest found, 99,90 PLN official store, Aug 2026) |
...
**Recommendation (decided)**: **ZBDongle-P** is the pick — the **CC2652P (Z-Stack) is the most mature, stable choice for Zigbee2MQTT**, and it's now affordable at **95 PLN (~22 EUR)** (cheapest found on Allegro; official store 99,90 PLN (~24 EUR)). The **NOUS E16 (49,99 PLN (~12 EUR))** stays the budget/multi-protocol fallback — but its **EFR32MG21/EmberZNet path is newer and less mature in Z2M** (drivers/firmware can be experimental/beta), so it's the value pick only if the ~45 PLN saving matters more than Z2M stability. Either way, add a plain **0.5–1 m USB extension cable** (keeps the dongle away from USB 3.0 interference).
...
- **Price — the E16 still saves ~45 PLN**: E16 49,99 PLN (~12 EUR) vs ZBDongle-P **95 PLN (~22 EUR) cheapest** (official 99,90 PLN (~24 EUR)) → a **~45 PLN** saving that can top up the Wyse 5070 budget (idea 05) — real, but Z2M stability still wins for a Z2M-centric lab.
...
**Verdict**: for this lab's Z2M-first architecture, the **ZBDongle-P (95 PLN) is the better buy** — mature and stable; the E16's ~45 PLN saving and Thread/BLE future-proofing are real but secondary to Z2M stability.
...
**Verdict**: the **ZBDongle-P** is the primary pick (Z2M stability; 95 PLN cheapest found); the **NOUS E16** is the budget/multi-protocol alternative with a less-mature EmberZNet-in-Z2M path. If the lab wants **central Zigbee placement without USB pass-through**, a **CC2652P2 network coordinator** is the strongest network option — same silicon family, Z2M over TCP, fully independent — at a higher price. The Sonoff Bridge Pro only makes sense as a flashed-DIY path; the Silvercrest gateway doesn't fit HA/Z2M at all.
...
2. **Coordinator**: settled on the **ZBDongle-P** (95 PLN (~22 EUR) cheapest / 99,90 PLN (~24 EUR) official) — CC2652P/Z-Stack is the most mature, stable path for Zigbee2MQTT; the **NOUS E16** (49,99 PLN (~12 EUR)) stays the budget multi-protocol fallback with a less-mature EmberZNet-in-Z2M path; a **CC2652P2 network coordinator** (RJ45/WiFi — central placement, no USB pass-through; see §6.1) stays an option — ties into the ZHA vs Z2M choice.
