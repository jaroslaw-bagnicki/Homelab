...
| `.devcontainer/scripts/setup-opencode.ps1` | Idempotent install of the OpenCode CLI binary via the official installer (`Invoke-WebRequest https://opencode.ai/install` → `& bash`); runs after persistence script so symlinks are in place before OpenCode writes its first-run config |
| `.devcontainer/devcontainer.json` | `postCreateCommand` chains `setup-opencode-persist.ps1` then `setup-opencode.ps1` then the existing setup scripts |
