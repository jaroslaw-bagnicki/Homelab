...
| 6 | 🧠 Configure DeepSeek V4 Pro custom provider | ADR written | ⬜ |
| 4 | 🐳 Test `devcontainer.json` in Local Sandbox | Copilot Desktop installed ✅ | ⬜ |
| 2 | 🔍 Spike: Research Azure auth for agentic workflows | Dev Container working | ⬜ |
| 5 | 🔑 Test GitHub MCP + Azure MCP connectivity | Azure auth spike | ⬜ |
| 7 | 🔐 Validate OS keychain secret storage | MCP working, API keys configured | ⬜ |
| 8 | 📋 Report findings → update ADR to `Accepted`/`Deferred` | All above complete | ⬜ |

**Already done:** Step 3 — Copilot Desktop Tech Preview installed on Windows.
...
### 4. Test Dev Container in Local Sandbox
...
### 2. Spike: Azure Auth for Agentic Workflows
...
### 7. Validate OS Keychain Secret Storage
