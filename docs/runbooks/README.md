...
| 17 | [17-opencode-server-on-cloudlab.md](17-opencode-server-on-cloudlab.md) | Per-project OpenCode server instances on Cloudlab — `docker_opencode_ingress` + `docker_opencode_instance` roles, KV-backed `OPENCODE_SERVER_PASSWORD`, wildcard `*-oc.cloud5.ovh` routing via dedicated `caddy-opencode` — see [ADR 18](../decisions/18-opencode-docker-sandbox.md) |
