...
| # | Runbook | Topic |
|---|---|---|
| 29 | [29-deploy-netdata.md](29-deploy-netdata.md) | Netdata Tier B — **Parent** host-native on the HA node's Proxmox (`ha`), Lab + Edge children streaming to it, via the shared `netdata` Ansible role; stream key from AKV — see [#104](https://github.com/jaroslaw-bagnicki/Homelab/issues/104) & [ADR 27](../decisions/27-monitoring-strategy.md) |
| 28 | [28-ha-proxmox-node.md](28-ha-proxmox-node.md) | HA node (Wyse 5070) — Proxmox VE install, static IP `192.168.2.201`, `fleetadm` bootstrap + base Ansible (`playbook-ha.yml` `common`→`security`); the Netdata Parent is provisioned separately ([runbook 29](29-deploy-netdata.md)) — see [issue #103](https://github.com/jaroslaw-bagnicki/Homelab/issues/103), [ADR 25](../decisions/25-home-assistant-thin-client.md), [research 29](../research/29-wyse5070-hardware-diagnostic.md) |
