...
| # | Runbook | Topic |
|---|---|---|
| 29 | [29-nut-ups-shutdown.md](29-nut-ups-shutdown.md) | UPS graceful shutdown via NUT — NUT server in **LXC 213** on the HA node (`nutdrv_qx` for the Green Cell UPSLM600, `0665:5161`), Proxmox host as the sole `upsmon` primary, `lab`/`edge` as secondaries (the ML110 is not joined to NUT; it retires once the Beetle array is verified), plus the two-strip power rewire and the modified-sine pull-the-plug test for the Beetle's active-PFC supply — see [idea 09](../ideas/09-ups-nut-home-assistant.md) & [issue #111](https://github.com/jaroslaw-bagnicki/Homelab/issues/111) |
| 28 | [28-ha-proxmox-node.md](28-ha-proxmox-node.md) | HA node (Wyse 5070) — Proxmox VE install, static IP `192.168.2.201`, `fleetadm` bootstrap + base Ansible (`playbook-ha.yml` `common`→`security`); the Netdata Parent is tracked separately ([#104](https://github.com/jaroslaw-bagnicki/Homelab/issues/104), [ADR 27](../decisions/27-monitoring-strategy.md)) — see [issue #103](https://github.com/jaroslaw-bagnicki/Homelab/issues/103), [ADR 25](../decisions/25-home-assistant-thin-client.md), [research 29](../research/29-wyse5070-hardware-diagnostic.md) |
