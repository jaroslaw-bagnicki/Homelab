...
| # | Runbook | Topic |
|---|---|---|
| 29 | [29-nut-ups-shutdown.md](29-nut-ups-shutdown.md) | UPS graceful shutdown via NUT — NUT server in **LXC 103** on the HA node (`nutdrv_qx` for the Green Cell UPSLM600, `0665:5161`), Proxmox host as the sole `upsmon` primary, `lab`/`edge` as secondaries (the ML110 is not joined to NUT; it retires once the Beetle array is verified), plus the two-strip power rewire and the modified-sine pull-the-plug test for the Beetle's active-PFC supply — see [idea 09](../ideas/09-ups-nut-home-assistant.md) & [issue #111](https://github.com/jaroslaw-bagnicki/Homelab/issues/111) |
