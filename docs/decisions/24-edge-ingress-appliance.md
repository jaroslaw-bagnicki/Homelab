...
- **Constrained hardware** — 2 GB RAM / 8 GB eMMC are soldered (no upgrade); zero headroom for full-size agents or Arc enrolment. Monitoring on the appliance must use **lightweight components** — e.g. a **Netdata child node** and **Fluent Bit** ([ADR 27](27-monitoring-strategy.md)) — and must **restrict caching/buffering to the eMMC drive**: only a small RAM buffer is allowed (Netdata `memory mode = ram`, no `dbengine` disk store; Fluent Bit in-memory buffering only). If headroom is exhausted, move to the Wyse 5070. Cherry Trail is aging.
