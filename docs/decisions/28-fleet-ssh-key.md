...
- `homelab` and `edge` — a public key pasted manually into `authorized_keys` during base install (runbooks 25 / 24) — the role the fleet key now fills at bootstrap.
...
- **Fleet key from day 1** — the breaking-glass account installs the fleet public key when it creates `labadmin`, so the fleet key is the first and only SSH credential on each host; Ansible and agents connect with it immediately, with no separate control key.
