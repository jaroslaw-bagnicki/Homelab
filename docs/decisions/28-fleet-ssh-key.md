...
- **Public key** — committed to the repo at `ansible/roles/common/files/ssh/ansible-fleet.pub` (public keys are not secrets) and deployed to `labadmin` on **every** host by the `common` role via `ansible.posix.authorized_key`, with restrictive `key_options` (`no-port-forwarding,no-agent-forwarding,no-X11-forwarding`) — Ansible and agents only need exec + sftp/scp.
