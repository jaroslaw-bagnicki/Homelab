...
> **Revised 2026-09-19:** Tier B is **LAN-fleet scoped** — `cloudlab` is never a Netdata target (Tier A already covers it); **7-day retention** bounded per tier by `dbengine tier N retention size`; agent updates are **opt-in** (`netdata_upgrade`); alarm *delivery* waits for the Home Assistant VM ([#68](https://github.com/jaroslaw-bagnicki/Homelab/issues/68)).
