...
> **Revised 2026-09-26:** the Parent also charts **UPS telemetry** — a bundled go.d `upsd` job polling the NUT server in LXC 213 (`netdata_upsd_address`), anonymous reads (no NUT account, no Key Vault secret) — realising [ADR 30](30-ups-nut-graceful-shutdown.md)'s "telemetry route into Netdata".
