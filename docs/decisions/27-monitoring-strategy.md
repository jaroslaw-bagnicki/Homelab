...
- **Transport (revised 2026-09-19)** — the Parent's `19999` listener is **HTTPS-only** (`^SSL=force`, plain HTTP refused) and a dedicated `19996` listener accepts **TLS-only** streams (`^SSL=force`; children append `:SSL`). The certificate is self-signed and generated on the node, and children do **not** verify it (`ssl skip certificate verification`), so the transport is encrypted against passive capture but not against an active MITM — pinning via `CAfile` is a tracked follow-up.
