...
> **Revised 2026-09-26 (retention):** retention is **per-tier**, no longer a flat 7 days — the Parent keeps **21 d at 1s (3 GiB cap)**, **30 d at 1m (2 GiB)** and **365 d at 1h (2 GiB)**, an ≈7 GiB ceiling sized from measured growth and held on the `pve` root LV, which the guests do not share; children keep **7 d / 256 MiB** per tier. The role parameter is now `netdata_retention_tiers` (a per-tier `time`/`size` list), replacing `netdata_retention_time` / `netdata_retention_size`.
