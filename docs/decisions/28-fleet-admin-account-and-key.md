...
Ansible manages three hosts today (ADR 10): the `cloudlab` Contabo VPS, the `lab` M910q, and the `edge` Wyse 3040 ingress appliance. The wider fleet also includes the OMV NAS (ADR 23), with a Home Assistant node (Proxmox VM, ADR 25) and an OPNsense router coming next. The Ansible-managed hosts share a generic **`fleetadm`** administration account (key-only SSH, NOPASSWD sudo, created at bootstrap as a new account, separate from the legacy `labadmin`), but SSH keys are distributed **imperatively and per-host**:
...
- **Fleet admin account** — the account is **`fleetadm`**: a shared, non-interactive fleet administration account with key-only SSH and NOPASSWD sudo, created as a **new account** by the break-glass account at bootstrap (runbooks 24/25) — separate from the legacy `labadmin`, which is left in place on already-provisioned hosts until they are migrated — and re-provisioned by Ansible. **Account + key are managed as one unit** — creating/retiring the account, rotating the key, or changing its sudo scope is a fleet-wide change applied by the playbook and runbooks together.
...
- **Account + key are one unit** — the `fleetadm` account and the fleet key travel together as the single fleet administration identity; already-provisioned hosts get `fleetadm` as a new account per [Migration](#migration), and `labadmin` is retired once the fleet is migrated.
...
### Migration (live hosts → `fleetadm`)

Already-provisioned hosts (`cloudlab`, `lab`, `edge`) get `fleetadm` as a **new account** — created by the break-glass console/account at bootstrap (runbooks 24/25), separate from the legacy `labadmin`, which is left in place to avoid lockout until the fleet is migrated. The `usermod -l` rename approach was rejected: it fails when the account owns the active session, and keeping the old login adds no value. New hosts bootstrap directly as `fleetadm` and skip this.

**Preconditions:** break-glass/console access to the host; `cloudlab` from the dev container, `lab`/`edge` from a LAN workstation.

Per host, as root/sudo:

```bash
useradd -m -s /bin/bash fleetadm
...
echo 'no-port-forwarding,no-agent-forwarding,no-X11-forwarding <fleet-public-key> fleetadm@homelab' > /home/fleetadm/.ssh/authorized_keys
chmod 600 /home/fleetadm/.ssh/authorized_keys
chown -R fleetadm:fleetadm /home/fleetadm
passwd -l fleetadm
```

Then run each host's playbook so the `common` role re-arms the key on `fleetadm` (`authorized_key` with `exclusive: true` — the fleet key is the sole credential). Verify with `ssh fleetadm@cloudlab "hostname && sudo whoami"` (→ `root`).

**Status:** `cloudlab` migrated 2026-08-30 (verified — see issue #94). `lab` and `edge` pending.
...
**Reference:** `scripts/New-HomelabFleetSshKey.ps1` · `ansible/roles/common/files/ssh/fleetadm.pub` · ADR [10](10-ansible-host-config.md) · ADR [16](16-agent-identity-pattern.md) · runbooks [24](../runbooks/24-edge-appliance.md) / [25](../runbooks/25-m910q-os-refresh.md)
