...
`pve`, and the incoming Beetle NAS) and the network appliances (switch, mesh node, LTE modem). Nothing
...
- **Its USB interface is `0665:5161`** (Cypress/INNO TECH bridge), whose HID report descriptor
  ...
  number**.
- **The `pve` node's Proxmox base is live** ([runbook 28](../runbooks/28-pve-proxmox-node.md)) while the
...
- **NUT server in a dedicated unprivileged LXC (213) on the `pve` node's Proxmox** — chosen over
