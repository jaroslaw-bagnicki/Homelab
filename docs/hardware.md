...
| Network | 1× GbE Intel I219-V (`enp0s31f6`, MAC `00:01:2e:8e:14:0d`) · DHCP — static `192.168.2.202` planned (ADR 31 / research 24) |
...
| Naming | OS hostname, Ansible and these docs address this node as `pve`; the **Proxmox node name** in the web UI is still `ha` — fixed at install, with no supported rename ([ADR 33](decisions/33-fleet-node-hostnames.md)) |
