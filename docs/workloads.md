...
- Adding a new workload:
  ...
  4. Add a row to `docs/overview.md` "What's Next", and a `CHANGELOG.md` entry on completion — the completing PR removes the row.
...
| NUT | `ansible/workloads/nut/nut-playbook.yml` | `nut-client` + `upsmon` across the fleet — `ha` as the sole `upsmon` primary, `lab`/`edge` as secondaries; monitor passwords from AKV, one shared `upsmon.conf` template. | [Workload README](../ansible/workloads/nut/README.md) · [Deploy — Runbook 30](runbooks/30-deploy-nut-clients.md) |
