...
A used **HP EliteDesk 800 G1 SFF** (LGA1150 / Q87 / Haswell) barebones, upgraded to a **Unraid** NAS with 3× 1 TB 2.5" HDDs + 1× M.2 NVMe cache. Chosen over the Fujitsu Q956 because the platform leaves room to grow:
...
| Component | Selection |
|---|---|
| **PC** | HP EliteDesk 800 G1 SFF barebones (no CPU/RAM/disk, 240W PSU) |
| **CPU** (not included — buy separately) | Intel Core i5-4570 / i5-4590 (4C/4T, 84W) — or i7-4770 / i7-4790 (4C/8T) for transcoding |
| **RAM** | 8 GB DDR3 PC3-12800 1600MHz (2× 4 GB kit); upgrade path to 16 GB (Unraid sweet spot) |
| **Disks** | 3× WD Blue 1 TB 2.5" HDD (WD10JUCX, 5400 RPM, 16 MB cache, SATA III) |
...
**Unraid** (paid license) — per the Gemini thread:
- Boots from a USB stick and runs entirely from RAM → the Q87 chipset's missing NVMe BIOS support is irrelevant
- NVMe via PCIe adapter = Cache Pool (appdata / Docker / VMs)
- 3× WD HDDs = parity array
...
- **SATA**: 4 on-board ports (2× SATA III 6Gbps + 2× SATA II 3Gbps) — enough for the 3 HDDs; an HBA unlocks expansion
...
### CPU & RAM (Unraid)
- SFF supports up to **84 W TDP**; avoid unlocked "K" chips (throttling) and **avoid "T" variants** (i5-4570T is only **2C/4T**, not 4C)
- **Economy**: i5-4570 / i5-4590 (4C/4T, ~30–50 zł)
- **Recommended (transcoding)**: i7-4770 / i7-4790 (4C/8T, ~100–140 zł); HD 4600 decodes H.264 but **not** H.265/HEVC 10-bit or AV1
...
- **Recommended**: LSI 9211-8i / 9207-8i (or Dell PERC H310 flashed to IT mode), ~100–140 zł, up to 8 disks via SFF-8087→4×SATA
...
- For single-client speed: 2.5 GbE card (Realtek RTL8125B, Low Profile, ~80–120 zł) if the switch supports 2.5G

### ⚠️ SMR caveat — the 3× WD 1 TB drives
- WD Blue 1 TB 2.5" (WD10JUCX) is an **SMR** drive — like most >1 TB 2.5" HDDs
- Unraid parity writes on SMR drives degrade drastically (to a few MB/s) — OK for a write-once backup target, but verify; CMR/SSD strongly preferred for a parity array (enable "Turbo Write" / Reconstruct Write if needed)
...
| LSI HBA IT mode (9211-8i / 9207-8i) | ✅ Recommended | Native Unraid support, up to 8 disks, ~100–140 zł |
| ASMedia ASM1064 controller | ⏸️ Budget option | Works, <2 W, no RAID — enough for the 3-disk build |
| i5-4570T ("T" 35W CPU) | ❌ Rejected | Only 2C/4T; no idle power savings vs full 4C i5 |
| Link aggregation 2× 1 GbE | ⏸️ Depends | Multi-client throughput only; not for single-client speed |

## Open Questions

- Which CPU to pair (i5 vs i7) — budget vs transcoding/parity-check headroom
- Extra SATA HBA needed? Only 3 HDDs → on-board SATA is sufficient for now
- Confirm WD10JUCX SMR behaviour under Unraid parity in practice
- Thermals/fan noise with 3 HDDs in the SFF case
...
| Spec | Value |
|---|---|
| Model / P/N | WD10JUCX (WD Blue) |
| Format | 2.5" |
| Capacity | 1 TB |
| RPM | 5400 |
| Cache | 16 MB |
| Interface | SATA III (6 Gbps) |
| Recording | SMR ⚠️ (verify) |
| Condition | New |

### Micron MTFDKBK256TFK-2450

| Spec | Value |
|---|---|
| Model | Micron MTFDKBK256TFK-2450 |
| Format | M.2 2230 |
| Capacity | 256 GB |
...
### UGREEN PCIe x16 → M.2 NVMe adapter

| Spec | Value |
|---|---|
| Brand / Model | UGREEN (offer 17379581052) |
| Type | Passive PCIe 4.0 x16 → M.2 NVMe adapter |
| Brackets | Full Height + **Low Profile** included |
| Condition | New |