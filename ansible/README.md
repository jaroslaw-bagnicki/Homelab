...
> **Control node per host.** `cloudlab` is managed from this dev container (see the `ansible-fleet-connect` skill); `lab` lives on the home LAN and is only reachable from a workstation on `192.168.2.0/24` — run the homelab playbook there (runbook 25).
