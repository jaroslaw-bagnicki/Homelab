...
Installs **NUT clients** (`nut-client` + `upsmon`) on the physical fleet nodes, so each node stops itself in an orderly way when the shared UPS battery runs down. It is the **client** half of the UPS/NUT setup: the NUT **server** runs in LXC 213 on the Proxmox VE host and is built by [runbook 29](../../../docs/runbooks/29-nut-ups-shutdown.md) §1–§3. Decision: [ADR 30](../../../docs/decisions/30-ups-nut-graceful-shutdown.md); tracked in [issue #116](https://github.com/jaroslaw-bagnicki/Homelab/issues/116).
...
- **The Proxmox VE host is the sole `primary`.** It waits `FINALDELAY 30` after the shutdown signal before stopping the Proxmox host, giving the secondaries time to finish; Proxmox then stops its own VMs/LXCs in order. Its overrides live in [`host_vars/pve.yml`](../../host_vars/pve.yml).
...
- `nut-upsmon-primary-password` — the Proxmox VE host, account `upsmon-host`
...
This keeps a from-scratch Proxmox VE rebuild (where the server container does not exist yet) from starting a fail-safe monitor with no server to reach.
...
| `nut_client_role` | `secondary` | `primary` on the Proxmox VE host (via `host_vars`). |
| `nut_client_account` | `upsmon-fleet` | `upsmon-host` on the Proxmox VE host. |
| `nut_client_password_secret` | `nut-upsmon-secondary-password` | `nut-upsmon-primary-password` on the Proxmox VE host. |
| `nut_client_hostsync` | `15` | `30` on the Proxmox VE host. |
| `nut_client_finaldelay` | `0` | `30` on the Proxmox VE host; inert on a secondary. |
...
- [ADR 30 — UPS graceful shutdown (NUT on the Proxmox VE node)](../../../docs/decisions/30-ups-nut-graceful-shutdown.md)
