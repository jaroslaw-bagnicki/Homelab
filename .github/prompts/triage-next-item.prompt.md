...
4. **Read-only. No writes at all** — no commit, push, PR, file write, or issue/label/board
   edit. You recommend; the operator applies, in a normal change — with a `CHANGELOG.md`
   entry only if it clears the notable bar in that file's header.
5. **Push back on the premise** when the honest answer is "finish X before starting anything".
6. **State the run date** (`YYYY-MM-DD`) — every relative window in the momentum rules
   ("last two weeks", "6+ weeks") depends on it.

**Argument.** A focus or constraint supplied with the invocation is a **hard filter** on the
candidate set — apply it *before* the momentum rules, and name the candidates it excluded.
With no argument, run the full pass.
...
Stop early only once **every** momentum rule can be evaluated — rules 1 and 7 need the
merged-PR history and issue ages, not just the board and CHANGELOG.
...
5. **Respect the WIP cap.** One operator. With ~3 rows already **In progress**, recommend
   finishing or parking one rather than adding a fourth — and name the destination: **Held**
   when the item is gated and has a known start date, **Not Scheduled** when it has none.
...
Open with the run date (`YYYY-MM-DD`), plus the constraint applied if one was supplied.
