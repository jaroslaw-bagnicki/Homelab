...
- **Open issues** (GitHub MCP, `jaroslaw-bagnicki/Homelab`) — the live candidate universe:
  ...
  Query `state: OPEN` and page through `pageInfo.endCursor` to exhaustion — an unfiltered list
  returns closed issues too, and a single page omits candidates.
...
- `CHANGELOG.md` and merged PRs — which workstreams are alive. Filter to merged records
  (`merged_at` set) and order by merge time; a plain PR list includes closed-but-unmerged ones.
  Current + previous month and ~15 PRs are a floor, not a limit — go back as far as the oldest
  live candidate requires, dormant work included.
...
| 6 | **Never pick gated work** | Test the **prerequisite itself** — hardware in hand, decision settled, dependency merged — not the presence of a "Waiting on" line, which may be stale. Say whether the gate is still live; an issue with no board row is not gated, so judge it on its own body |
...
**2. Hygiene.** One line each, seeded by the `docs/reports/*-quality-assessment.md` report —
a point-in-time snapshot, so take it as a **shortlist only** and re-check every entry against
current issue, board and doc state, dropping anything already fixed or no longer true. Then:
unlabelled issue or `(type)`-prefixed title · superseded by a settled ADR but still open ·
stale (6+ weeks) or duplicate · `overview.md` and `hardware.md` disagreeing on **any** node's
status, checked node by node rather than only for the shortlist.
